...
class EDA:
    def __init__(self):
        ...
        with tabs[9]:
            ...
            selected_region = st.selectbox("Select Region", ['전국'] + sorted(df['지역'].unique()))
            filtered = df[df['지역'] == selected_region]

            fig, ax = plt.subplots()
            ax.plot(filtered['연도'], filtered['인구'], marker='o')
            ax.set_title(f"Population Trend - {selected_region}")
            ax.set_xlabel("Year")
        ...
        # 10. 지역별 분석
        with tabs[10]:
            st.subheader("Top 5-Year Regional Changes")

            recent = df[df['지역'] != '전국']
            max_year = recent['연도'].max()
            min_year = max_year - 5

            pivot = recent.pivot(index='지역영문', columns='연도', values='인구')
            pivot['Change'] = (pivot[max_year] - pivot[min_year]) / 1000
            pivot['ChangeRate'] = ((pivot[max_year] - pivot[min_year]) / pivot[min_year]) * 100
            pivot = pivot.sort_values(by='Change', ascending=False).reset_index()

            # Change Graph
            fig1, ax1 = plt.subplots(figsize=(10, 6))
            sns.barplot(data=pivot, y='지역영문', x='Change', ax=ax1)
            ...
            sns.barplot(data=pivot, y='지역영문', x='ChangeRate', ax=ax2)
            for i, v in enumerate(pivot['ChangeRate']):
        ...
        # 12. 변화량 분석
        with tabs[12]:
            st.subheader("Top 100 Year-Region Changes")
            df_change = df[df['지역'] != '전국'].copy()
            df_change.sort_values(by=['지역영문', '연도'], inplace=True)
            df_change['증감'] = df_change.groupby('지역영문')['인구'].diff()
            top100 = df_change.sort_values(by='증감', key=abs, ascending=False).head(100)
            top100['인구'] = top100['인구'].map('{:,.0f}'.format)
            top100['증감'] = top100['증감'].map('{:,.0f}'.format)

            st.dataframe(
                top100[['연도', '지역영문', '인구', '증감']].style.background_gradient(subset='증감', cmap='RdBu_r')
            )
        ...
        with tabs[13]:
            ...
            pivot_viz = df_viz.pivot_table(index='연도', columns='지역영문', values='인구', aggfunc='sum')

            fig, ax = plt.subplots(figsize=(12, 6))
            pivot_viz.plot.area(ax=ax, colormap='tab20', linewidth=0)
            ax.set_title("Population by Region over Time")
            ax.set_xlabel("Year")
            ax.set_ylabel("Population")
            ax.legend(title="Region", bbox_to_anchor=(1.0, 1.0), loc='upper left')
            st.pyplot(fig)
            st.caption("Each region's contribution to population over time (excluding national total).")
